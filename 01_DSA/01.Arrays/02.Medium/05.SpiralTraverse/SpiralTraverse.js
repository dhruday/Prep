...
function spiralTraverse(array) {
    ...
    while (startRow <= endRow && startColumn <= endColumn) {
        for (let column = startColumn; column <= endColumn; column++) result.push(array[startRow][column]);
        for (let row = startRow + 1; row <= endRow; row++) result.push(array[row][endColumn]);

        if (startRow < endRow && startColumn < endColumn) {
            for (let column = endColumn - 1; column > startColumn; column--) result.push(array[endRow][column]);
            for (let row = endRow; row > startRow; row--) result.push(array[row][startColumn]);
        }
