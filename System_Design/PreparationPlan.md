...
- From your index: Backend `Part21` #300–306 (Two Pointers, Sliding Window, Frequency Maps)
...
**This directly maps to:** Backend Part24 #212 (Idempotent APIs) which you've just written
...
**Connect to your Spring notes:** Your `211` (ORM), `209` (@Transactional) — async + DB interactions
...
**Why this week:** Adobe asks about CRP. Google asks about browser internals in senior SWE rounds.
...
- Implement from scratch: LRU Cache (#318), EventEmitter (#319), deep clone (#320)
...
- Theory: Frontend #300–303 from your index
...
- Frontend #310–312: BFS/DFS templates, binary tree traversals
- DOM tree traversal as graph problem (#314) — unique to frontend interviews

---

### Day 38 — Graphs + Connected Components
- 4 LeetCode mediums: Number of islands, course schedule, graph BFS
- Frontend #313: Graph connected components template

---

### Day 39 — Stacks + Queues
- 4 LeetCode mediums: Monotonic stack, valid parentheses, LRU cache
- Frontend #307–309, #318 (LRU implementation)
- **LRU Cache from scratch** — doubly linked list + HashMap — appears in every big tech company

---

### Day 40 — Dynamic Programming
- 4 LeetCode mediums/hards: Knapsack variant, longest common subsequence, coin change
- Frontend #315–317: DP mental models, memoization vs tabulation

---

### Day 41 (Sat) — Frontend-Specific Implementations
- Implement from scratch (no reference):
  - `Promise.all` + `Promise.race` + `Promise.allSettled` (#321)
  - `curry`, `memoize`, `once`, `pipe` (#322)
  - `EventEmitter` pub/sub (#319)
  - `deepClone` + `deepEqual` (#320)
...
- Your system design answers should mention Bigtable, Spanner, Pub/Sub equivalents when relevant
...
**Why this matters:** When interviewing at Google/Adobe/Microsoft, your SAP experience needs translating. This day ensures you can speak to it fluently.
...
**SECONDARY:** Re-read your weakest Part24 Spring file (whichever internals you're least confident on)
